...
{
	 hint "Vous êtes dans une safezone! Vous ne pouvez pas lockpick les véhicules.";
};
...
_title = format[localize "Infraction du %1 en cours...",if(!_isVehicle) then {"Handcuffs"} else {getText(configFile >> "CfgVehicles" >> (typeOf _curTarget) >> "displayName")}];
