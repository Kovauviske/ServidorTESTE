...
if (_speed > _limit) then {
	if (side _driver == civilian) then {
		...
		if(license_civ_driver OR license_civ_truck ) then {
			...
			if(life_speedTicket <= 3) then {systemChat format["Você recebeu uma multa de numero %1 , respeite o limite de velocidade",life_speedTicket];};
			...
			if(life_speedTicket > 4 && life_speedTicket < 8) exitWith {
					...
					hint parseText format ["<t color='#ffffff'><t size='2'><t align='center'>Bilhete de estacionamento<br/><t color='#ff0000'><t align='center'><t size='1.5'>Velocidade: %1 km/h<br/><t color='#ffffff'><t align='center'><t size='1'>Velocidade Max: %2 km/h<br/><t color='#ffffff'><t align='center'><t size='1'>Motorista: %3<br/><t color='#ffffff'><t align='center'><t size='1'>Você tem varias multas de velocidade, você vai perder suas licenças e agora ta na lista de procurados",round _speed,_limit,name _driver];
			...
			if(life_atmcash <= _ticket) then
			{
				...
				{
					hint parseText format ["<t color='#ffffff'><t size='2'><t align='center'>Radar de Velocidade<br/><t color='#ff0000'><t align='center'><t size='1.5'>Velocidade: %1 km/h<br/><t color='#ffffff'><t align='center'><t size='1'>Velocidade MAX: %2 km/h<br/><t color='#ffffff'><t align='center'><t size='1'>Motorista: %3<br/><t color='#ffffff'><t align='center'><t size='1'>Valor da Multa: $%4<br/>Você está falido e não tem dinheiro para pagar, agora você tem um mandado de prisão",round _speed,_limit,name _driver,[_ticket] call life_fnc_numberText];
					[[getPlayerUID _driver,name _driver,"120S"],"life_fnc_wantedAdd",false,false] spawn life_fnc_MP;
				};
				life_atmcash = life_atmcash - _ticket;
				hint parseText format ["<t color='#ffffff'><t size='2'><t align='center'>Radar de Velocidade<br/><t color='#ff0000'><t align='center'><t size='1.5'>Velocidade: %1 km/h<br/><t color='#ffffff'><t align='center'><t size='1'>Velocidade MAX: %2 km/h<br/><t color='#ffffff'><t align='center'><t size='1'>Motorista: %3<br/><t color='#ffffff'><t align='center'><t size='1'>Valor da Multa: $%4",round _speed,_limit,name _driver,[_ticket] call life_fnc_numberText];
