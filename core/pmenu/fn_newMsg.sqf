...
switch(_type) do
{
	...
	case 1:
	{
		if(isNUll life_smartphoneTarget) exitWith {hint format["Selecione uma pessoa"];};
	...
	case 2:
	{
		if(({side _x == west} count playableUnits) == 0) exitWith {hint format["A polícia não está acessível no momento. Por favor, tente novamente."];};
		...
		_to = "The Police";
		hint format["Você enviou para %1 uma mensagem: %2",_to,_msg];
	...
	case 4:
	{
		...
		hint format["Você enviou uma msg para todos os SAMU",_msg];
	...
	case 5:
	{
		...
		hint format["Admin Mensagem PARA: %1 - Mensagem: %2",name life_smartphoneTarget,_msg];
		closeDialog 88883;
	};
	//emergencyloading
	case 6:
	...
	case 7:
	{
		...
		hint format["ADMIN MENSAGEM PARA TODOS: %1",_msg];
		closeDialog 887890;
	};
	//adacrequest
	case 8:
	{
		if(({side _x == independent} count playableUnits) == 0) exitWith {hint format["Atualmente nenhum medico está online"];};
		...
		hint format["Chamado de emergencia foi enviado",_msg];
