...
switch(true) do
{
	case (_rand <= 50): //30% de chance, changement de bourse par défaut
	{
		{
			if(random(10) <= 4) then //Random for each resource
			{
				...
				_modifier = [-20,20] call life_fnc_randomRound; //augmentation/diminution
	...
	case (_rand <= 65): //15% de chance, interpol arrete des trafiquants
	{
		[[3,"Interpol a arrêté un groupe de trafiquants de drogue!"],"life_fnc_broadcast",true,false] spawn life_fnc_MP;
	...
	case (_rand <= 75): //10% Crise pétrolière
	{
		[[3,"Un pétrolier vient de couler!"],"life_fnc_broadcast",true,false] spawn life_fnc_MP;
		
		["oilp", [20,40] call life_fnc_randomRound, true] call life_fnc_marketBuy; 
		
		diag_log "+Market+ Event oilp";
	};
	case (_rand <= 80): //15% World-China-Center Ciment, fer et vitres
	{
		[[3,"La chine lance le chantier du World-China-Center!"],"life_fnc_broadcast",true,false] spawn life_fnc_MP;
	...
	case (_rand <= 95): //10% Marijuana médicinale
	{
		[[3,"La marijuana médicinale est de plus en plus rare!"],"life_fnc_broadcast",true,false] spawn life_fnc_MP;
	...
	case (_rand <= 95): //10% La chine veux des tortues 
	{
		[[3,"La Chine a découvert les tortues comme un nouveau médicament!"],"life_fnc_broadcast",true,false] spawn life_fnc_MP;
