...
_toKeep = [_this,0,FALSE,[TRUE]] call BIS_fnc_param;
if(!((_vehicle isKindOf "Car") || (_vehicle isKindOf "Air") || (_vehicle isKindOf "Ship")  || (typeof _vehicle in life_contenant))) exitWith {};
if(player distance cursorTarget > 10) exitWith {};
if ((side player == civilian) && (_vehicle in life_vehicles)) exitwith {hint "Vous ne pouvez pas mettre en fourrière votre propre véhicule!"};
if((_vehicle isKindOf "Car") || (_vehicle isKindOf "Air") || (_vehicle isKindOf "Ship")) then
{
	...
	if (_toKeep) then 
	{
		[[0,"STR_NOTF_BeingImpounded_keep",true,[(_vehicleData select 0) select 1,_vehicleName]],"life_fnc_broadcast",true,false] spawn life_fnc_MP;
	}
	else
	{
	...
	_upp = if (_toKeep) then 
	{
		localize "STR_NOTF_Impounding_keep";
	}
	else
	{
	...
	if(player distance _vehicle > 10) exitWith 
	{
		if (_toKeep) then 
		{
			hint localize "STR_NOTF_ImpoundingCancelled_keep";
		}
		else
		{
	...
	switch (true) do
	{
		...
		case (typeof _vehicle in life_contenant) : {_price = (call life_impound_car);};
		case (_vehicle isKindOf "Ship"): {_price = (call life_impound_boat);};
		case (_vehicle isKindOf "Air"): {_price = (call life_impound_air);};
	};
	
	life_impound_inuse = true;
	[[_vehicle,true,player,_toKeep],"TON_fnc_vehicleStore",false,false] spawn life_fnc_MP;
	waitUntil {!life_impound_inuse};
	if (_toKeep) then
	{
		hint format[localize "STR_NOTF_Impounded_keep",_type,_price];
		[[0,"STR_NOTF_HasImpounded_keep",true,[profileName,(_vehicleData select 0) select 1,_vehicleName]],"life_fnc_broadcast",true,false] spawn life_fnc_MP;
	}
	else
	{
