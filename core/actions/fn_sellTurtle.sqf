...
if(life_inv_turtle == 0) exitWith {
	titleText["Você não tem nenhuma Tartaruga para vender","PLAIN"];
};
...
if([false,"turtle",life_inv_turtle] call life_fnc_handleInv) then {
	titleText[format["Você vendeu %1 tartarugas por %2 R$",_val,[_price] call life_fnc_numberText],"PLAIN"];
