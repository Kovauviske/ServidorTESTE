...
if ((_type == "uranium2") && (life_inv_puranium == 0)) exitWith {hint "Você não tem nenhum Uranio";};

//unprocessed item,processed item, cost if no license,Text to display (I.e Processing (percent) ..."
_itemInfo = switch (_type) do
{
    case "uranium2": {["uranium2","puranium","uranium3",6000,"Uranio dissolvido Legal"];};
    case "uranium2b": {["uranium2","ipuranium","uranium3",6000,"Uranio dissolvido Ilegal"];};
    default {[]};
};
...
if(_hasLicense) then
{
    ...
    if(player distance _vendor > 10) exitWith {hint "Você precisa está a 10m para processar"; 5 cutText ["","PLAIN"]; life_is_processing = false;};
    ...
    else
{
    ...
    if(life_cash < _cost) exitWith {hint format["Você precisa de %1 R$ para licença",[_cost] call life_fnc_numberText]; 5 cutText ["","PLAIN"]; life_is_processing = false;};
    ...
    if(!([true,_newItem,_oldVal] call life_fnc_handleInv)) exitWith {5 cutText ["","PLAIN"]; [true,_oldItem1,_oldVal] call life_fnc_handleInv; [true,_oldItem2,_oldVal] call life_fnc_handleInv; life_is_processing = false;};
    5 cutText ["","PLAIN"];
    titleText[format["Você processou %1 e %2 E ganhou %3 R$",_oldVal,_itemName,[_cost] call life_fnc_numberText],"PLAIN"];
