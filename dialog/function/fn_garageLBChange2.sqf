...
_retrievePrice = [_className,__GETC__(life_garage_prices2)] call TON_fnc_index;
...
_retrievePrice = if(_retrievePrice == -1) then {1000} else {(__GETC__(life_garage_prices2) select _retrievePrice) select 1;};
