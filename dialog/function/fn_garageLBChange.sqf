...
if (life_is_keeped) then
{
	_retrievePrice = if(_retrievePrice == -1) then {1000} else {
	((__GETC__(life_garage_prices) select _retrievePrice) select 1) * life_fouriere_modifier
	};
}
else
{
